...
saved_model = torch.load("gatys/models/transform_network.pth", map_location=torch.device('cpu'))
...
#                                      transforms.ToTensor()))
#     input_tensor = torch.unsqueeze(preprocess(image1), 0)
#     input_tensor = torch.unsqueeze(preprocess(image2), 0)
#     output_tensor = saved_model(Variable(input_tensor.type('torch.FloatTensor >
#     generated_image=output_tensor.detach().squeeze().permute(1, 2, 0).numpy >
    ...
    with gr.Tab("Model1"):
        with gr.Row():
            ...
            with gr.Column():
                ...
                styles=gr.Examples(examples=["styles/Arles.jpg", "styles/starryNight.jpg", "styles/theMuse.jpg",
                                   "styles/UnderTheWave.jpg"], inputs=[image_input1], label="Style Image")
    ...
    with gr.Tab("Model2"):
        with gr.Row():
            ...
            with gr.Column():
                gr.Markdown(
                    "### This models's style must be chosen in these 4 styles:")
                styles2=gr.Examples(examples=["styles/Arles.jpg", "styles/starryNight.jpg", "styles/theMuse.jpg",
                                    "styles/UnderTheWave.jpg"], inputs=[image_input3], label="Style Image")
